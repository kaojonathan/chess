				...
				{ // iterate over that piece's possible moves
					...
					{ // if the same enemy piece can't attack that piece's possible move position, move it
						...
						{								// if it's move returns 1 (normal)
							...
							gameBoard->moveP(pieces[i]->getX(), pieces[i]->getY(), pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second); // move the piece on the board; return pair with (1, "normal")
						...
						{																															  // if it's move returns 2 (capture)
							std::string capturedRep = gameBoard->getPiece(pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second)->getRep(); // get the captured type
							opponent->removePiece(std::pair<int, int>{pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second});			  // remove the piece from opponent
							oldCol = pieces[i]->getX();																								  // update the parameters with the move positions
							oldRow = pieces[i]->getY();
							newCol = pieces[i]->getMoves()[k].first;
							newRow = pieces[i]->getMoves()[k].second;
							gameBoard->moveP(pieces[i]->getX(), pieces[i]->getY(), pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second); // move the piece on the board; return pair with (2, capturedRep)
						...
						{								// if it's move returns 4 (promotion)
							...
							newCol = pieces[i]->getMoves()[k].first;
							newRow = pieces[i]->getMoves()[k].second;
							gameBoard->moveP(pieces[i]->getX(), pieces[i]->getY(), pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second); // move the piece on the board
							deletePiece(std::pair<int, int>(pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second));						 // delete piece from player; return pair with (4, "promotion")
						...
						{																															  // if it's move returns 5 (promotion with capture)
							...
							newRow = pieces[i]->getMoves()[k].second;
							gameBoard->moveP(pieces[i]->getX(), pieces[i]->getY(), pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second); // move the piece on the board
							deletePiece(std::pair<int, int>(pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second));						 // delete piece from player; return pair with (5, capturedRep)
						...
						{								// if it's move returns 6 (enPassant)
							...
							{																													 // if white pawn
								opponent->removePiece(std::pair<int, int>{pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second + 1}); // remove the piece from opponent
								gameBoard->captureP(pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second + 1);						 // capture the piece
							}
							...
							gameBoard->moveP(pieces[i]->getX(), pieces[i]->getY(), pieces[i]->getMoves()[k].first, pieces[i]->getMoves()[k].second); // move the piece on the board; return pair with (6, "enPassant")
			...
			{ // iterate over that piece's possible moves
				...
				{ // if the same enemy piece can't attack that piece's possible move position, move it
					...
					{						   // if it's move returns 1 (normal)
						...
						gameBoard->moveP(king->getX(), king->getY(), king->getMoves()[k].first, king->getMoves()[k].second); // move the piece on the board; return pair with (1, "normal")
						return pair<int, std::string>{1, "normal"};
					}
					else if (king->move(king->getMoves()[k].first, king->getMoves()[k].second) == 2)
					{
						std::string capturedRep = gameBoard->getPiece(king->getMoves()[k].first, king->getMoves()[k].second)->getRep(); // get the captured type
						opponent->removePiece(std::pair<int, int>{king->getMoves()[k].first, king->getMoves()[k].second});				// remove the piece from opponent
						oldCol = king->getX();																							// update the parameters with the move positions
						oldRow = king->getY();
						newCol = king->getMoves()[k].first;
						newRow = king->getMoves()[k].second;
						gameBoard->moveP(king->getX(), king->getY(), king->getMoves()[k].first, king->getMoves()[k].second); // move the piece on the board; return pair with (2, capturedRep)
						return pair<int, std::string>{2, capturedRep};
					}
					...
					{						   // if it's move returns 4 (promotion)
						...
						newCol = king->getMoves()[k].first;
						newRow = king->getMoves()[k].second;
						gameBoard->moveP(king->getX(), king->getY(), king->getMoves()[k].first, king->getMoves()[k].second); // move the piece on the board
						deletePiece(std::pair<int, int>(king->getMoves()[k].first, king->getMoves()[k].second));			 // delete piece from player; return pair with (4, "promotion")
						return pair<int, std::string>{4, "promotion"};
					}
					...
					{																													// if it's move returns 5 (promotion with capture)
						...
						newRow = king->getMoves()[k].second;
						gameBoard->moveP(king->getX(), king->getY(), king->getMoves()[k].first, king->getMoves()[k].second); // move the piece on the board
						deletePiece(std::pair<int, int>(king->getMoves()[k].first, king->getMoves()[k].second));			 // delete piece from player; return pair with (5, capturedRep)
						return pair<int, std::string>{5, capturedRep};
					}
				}
			}
